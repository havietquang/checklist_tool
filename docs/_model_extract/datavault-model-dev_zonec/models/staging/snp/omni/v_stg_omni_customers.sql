...
{{ config(
    alias = 'v_stg_omni_customers',
    materialized = 'view',
    tags = ['omni', 'customer', 'phase1', 'all']
) }}
...
========================================================================
  ...
  - source_table          : Ten bang nghiep vu nguon ('customers'),
                            dung de map dung snapshot/source table.
  - business_key_cols     : Danh sach cot tao thanh Business Key duy nhat
                            cua entity. ['cif']
                            Macro se hash cac cot nay thanh hashkey.
  - source_event_date_col : Cot ngay su kien tu nguon ('created_at'),
...
{% set business_key_cols = ['cif'] -%}
{% set staging_config = get_staging_config(source_name, source_table) %}
{% set source_event_date_col = staging_config.source_event_date_col %}
{% set source_event_date_dttype = staging_config.source_event_date_dttype %}
{% set hashdiff_satellite_dict =
{
    'hashdiff_omni_customer_information': ['created_at','customer_status']
}
-%}
