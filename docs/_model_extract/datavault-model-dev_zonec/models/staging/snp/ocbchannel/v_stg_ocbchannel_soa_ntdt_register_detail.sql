...
{{ config(
    alias = 'v_stg_ocbchannel_soa_ntdt_register_detail',
    materialized = 'view',
    tags = ['ocbchannel', 'soa_ntdt_register_detail', 'zonec']
) }}
...
========================================================================
  ...
  - source_table          : Ten bang nghiep vu nguon ('soa_ntdt_register_detail'),
                            dung de map dung snapshot/source table.
  - business_key_cols     : Danh sach cot tao thanh Business Key cua entity.
                            ['register_id'] - hash dong nhat voi
                            hub_soa_ntdt_register (nhanh union tu detail).
  - ma_key_expr           : Bieu thuc sinh cot ma_key (multi-active key).
                            = bank_account: mot register co nhieu tai khoan.
...
{% set business_key_cols = ['register_id'] -%}
{% set ma_key_expr = "src.bank_account" -%}
{% set staging_config = get_staging_config(source_name, source_table) %}
{% set source_event_date_col = staging_config.source_event_date_col %}
{% set source_event_date_dttype = staging_config.source_event_date_dttype %}
{% set hashdiff_satellite_dict =
{
    'hashdiff_soa_ntdt_register_bank_account': ['ma_key', 'bank_account'],
}
-%}
...
ben canh cac cot chuan: hashkey, hashdiff_full, hashdiff satellites,
record_source, source_event_date, load_timestamp.
------------------------------------------------------------------------
*/
