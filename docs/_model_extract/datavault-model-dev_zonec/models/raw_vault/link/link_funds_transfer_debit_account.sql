...
*/
{{ config(
    alias = 'link_funds_transfer_debit_account',
    materialized = 'incremental_checkpoint',
    incremental_strategy = 'merge',
    unique_key = ['link_funds_transfer_debit_account_hashkey'],
...
{% set unique_key = 'link_funds_transfer_debit_account_hashkey' %}

{% set raw_sql %}
SELECT
    {{ hash_column(['id', 't_debit_acct_no'], source_name) }} AS {{ unique_key }},
    {{ hash_column(['id'], source_name) }} AS funds_transfer_hashkey,
    {{ hash_column(['t_debit_acct_no'], source_name) }} AS account_hashkey,
    to_date('{{ var("target_date") }}', 'yyyyMMdd') AS source_event_date,
    CONCAT('{{ source_name }}', '__', '{{ source_table }}') AS record_source,
    current_timestamp AS load_timestamp
FROM {{ ref(source_model) }}
WHERE source_event_date = to_date('{{ var("target_date") }}', 'yyyyMMdd')
AND t_debit_acct_no IS NOT NULL
AND id IS NOT NULL
AND t_debit_acct_no NOT LIKE 'PL%'
{% endset %}
