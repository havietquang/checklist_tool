...
*/
{{ config(
    alias = 'link_teller_account2',
    materialized = 'incremental_checkpoint',
    incremental_strategy = 'merge',
    unique_key = ['link_teller_account2_hashkey'],
...
{% set unique_key = 'link_teller_account2_hashkey' %}
...
(
    select distinct
        id,
        explode(split(t_account_2, "::")) as t_account_2,
        to_date('{{ var("target_date") }}', 'yyyyMMdd') as source_event_date
    from {{ ref(source_model) }}
    where source_event_date = to_date('{{ var("target_date") }}', 'yyyyMMdd')
)
select
    {{ hash_column(['t_account_2', 'id'], source_name) }} as link_teller_account2_hashkey,
    {{ hash_column(['id'], source_name) }} as teller_hashkey,
    {{ hash_column(['t_account_2'], source_name) }} as account_hashkey,
    source_event_date,
    concat('{{ source_name }}', '__', '{{ source_table }}') as record_source,
    current_timestamp as load_timestamp
from hash_value
where id is not null
and t_account_2 is not null
and t_account_2 not like 'PL%'
{%- endset %}
