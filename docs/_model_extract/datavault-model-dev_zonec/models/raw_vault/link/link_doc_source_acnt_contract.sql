...
*/
{{ config(
    alias = 'link_doc_source_acnt_contract',
    materialized = 'incremental_checkpoint',
    incremental_strategy = 'merge',
    unique_key = ['link_doc_source_acnt_contract_hashkey'],
...
{% set source_business_key_cols = ['doc.ID', 'doc.SOURCE_CONTRACT'] %}
{% set document_business_key_cols = ['doc.ID'] %}
{% set acnt_contract_business_key_cols = ['doc.SOURCE_CONTRACT'] %}

{% set raw_sql %}
SELECT
    {{ hash_column(source_business_key_cols, source_name) }} AS link_doc_source_acnt_contract_hashkey,
