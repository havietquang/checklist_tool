...
*/
{{ config(
    alias = 'link_voucher_campaign',
    materialized = 'incremental_checkpoint',
    incremental_strategy = 'merge',
    unique_key = ['link_voucher_campaign_hashkey'],
...
{% set unique_key = 'link_voucher_campaign_hashkey' %}
...
SELECT
        vc.hashkey AS link_voucher_campaign_hashkey,
        {{ hash_column(['vc.voucher_campaign_code'], source_name) }} as voucher_campaign_hashkey,
        {{ hash_column(['v.voucher_code'], source_name) }} as voucher_hashkey,
        to_date('{{ var("target_date") }}', 'yyyyMMdd') as source_event_date,
        CONCAT(CAST('omni' AS string), '__', 'voucher_campaign') AS record_source,
        CAST(current_timestamp AS timestamp) AS load_timestamp
    FROM {{ ref('v_stg_omni_voucher_campaign') }} vc
    join {{ ref('v_stg_omni_voucher') }} v
...
{{ link(
    ...
    raw_sql = raw_sql,
    source_business_key_cols = ['voucher_campaign_code', 'voucher_code'],
    foreign_business_key_cols = {
        'voucher_campaign_hashkey': ['voucher_campaign_code'],
        'voucher_hashkey': ['voucher_code']
    }
) }}
