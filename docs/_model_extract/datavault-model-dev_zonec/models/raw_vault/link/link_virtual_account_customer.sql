...
*/
{{ config(
    alias = 'link_virtual_account_customer',
    materialized = 'incremental_checkpoint',
    incremental_strategy = 'merge',
    unique_key = ['link_virtual_account_customer_hashkey'],
...
{% set unique_key = 'link_virtual_account_customer_hashkey' %}
...
{% set raw_sql %}
SELECT
    sha2(COALESCE(TRIM(CAST(id AS string)), '') || '$' || COALESCE(UPPER(TRIM(CAST(t_cif AS string))), ''), 256) AS link_virtual_account_customer_hashkey,
