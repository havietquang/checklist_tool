...
*/
{{ config(
    alias = 'link_payment_order_funds_transfer',
    materialized = 'incremental_checkpoint',
    incremental_strategy = 'merge',
    unique_key = ['link_payment_order_funds_transfer_hashkey'],
...
{% set payment_order_business_key_cols = ['a.id'] %}
...
payment_order as (
    ...
    from {{ source('omni', 'payment_order') }}
...
SELECT
    {{ hash_column(['a.id', 'b.id'], source_name) }} AS link_payment_order_funds_transfer_hashkey,
    {{ hash_column(payment_order_business_key_cols, source_name) }} AS payment_order_hashkey,
