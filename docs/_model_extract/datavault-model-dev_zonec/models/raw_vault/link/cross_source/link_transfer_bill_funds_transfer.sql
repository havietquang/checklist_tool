...
*/
{{ config(
    alias = 'link_transfer_bill_funds_transfer',
    materialized = 'incremental_checkpoint',
    incremental_strategy = 'merge',
    unique_key = ['link_transfer_bill_funds_transfer_hashkey'],
...
{% set transfer_bill_business_key_cols = ['a.id'] %}
...
transfer_bill as (
    ...
    select id, ft_trans_no, data_date
    from {{ source('omni', 'transfer_bill_history') }}
...
SELECT
    {{ hash_column(['a.id', 'b.id'], source_name) }} AS link_transfer_bill_funds_transfer_hashkey,
...
FROM transfer_bill a
LEFT JOIN funds_transfer b ON a.ft_trans_no = b.funds_transfer_id
WHERE a.ft_trans_no is not null
