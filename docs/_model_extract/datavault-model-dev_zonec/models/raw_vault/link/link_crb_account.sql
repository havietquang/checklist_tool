...
*/
{{ config(
    alias = 'link_crb_account',
    materialized = 'incremental_checkpoint',
    incremental_strategy = 'merge',
    unique_key = ['link_crb_account_hashkey'],
...
{% set account_business_key_cols = ['b.ID'] %}

{%- set raw_sql -%}
SELECT DISTINCT
    {{ hash_column(source_business_key_cols,source_name) }} AS link_crb_account_hashkey,
...
JOIN {{ ref('v_stg_t24_t24_account') }} b
    ON a.tieukhoan = b.id
WHERE a.source_event_date = to_date('{{ var("target_date") }}', 'yyyyMMdd') and b.source_event_date = to_date('{{ var("target_date") }}', 'yyyyMMdd')
