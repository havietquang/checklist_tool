...
*/
{{ config(
    alias = 'link_product_package_partner_customer',
    materialized = 'incremental_checkpoint',
    incremental_strategy = 'merge',
    unique_key = ['link_product_package_partner_customer_hashkey'],
...
{% set unique_key = 'link_product_package_partner_customer_hashkey' %}
...
{% set raw_sql %}
SELECT
    sha2(COALESCE(TRIM(CAST(id AS string)), '') || '$' || COALESCE(UPPER(TRIM(CAST(t_cif AS string))), ''), 256) AS link_product_package_partner_customer_hashkey,
    {{ hash_column(['id'], source_name, false) }} AS product_package_partner_hashkey,
    {{ hash_column(['t_cif'], source_name) }} AS customer_hashkey,
