...
*/
{{ config(
    alias = 'link_voucher_campaign_user_group',
    materialized = 'incremental_checkpoint',
    incremental_strategy = 'merge',
    unique_key = ['link_voucher_campaign_user_group_hashkey'],
    skip_matched_step = true,
    tags = ['omni', 'phase1', 'all']
) }}
...
{% set unique_key = 'link_voucher_campaign_user_group_hashkey' %}
...
{{ link(
    ...
    source_business_key_cols = ['voucher_campaign_code', 'voucher_user_group_id'],
    foreign_business_key_cols = {
        'voucher_campaign_hashkey': ['voucher_campaign_code'],
        'user_group_hashkey': ['voucher_user_group_id']
    }
) }}
