...
*/
{{ config(
    alias = 'link_balance_acnt_contract',
    materialized = 'incremental_checkpoint',
    incremental_strategy = 'merge',
    unique_key = ['link_balance_acnt_contract_hashkey'],
...
{% set source_model = 'v_stg_way4_balance' %}
{% set source_name = 'way4' %}
{% set source_table = 'oms_balance' %}
{% set unique_key = 'link_balance_acnt_contract_hashkey' %}
{% set source_business_key_cols = ['b.id', 'b.acnt_contract_id'] %}
{% set balance_business_key_cols = ['b.id'] %}
...
SELECT
    {{ hash_column(source_business_key_cols, source_name) }} AS link_balance_acnt_contract_hashkey,
    {{ hash_column(balance_business_key_cols, source_name) }} AS balance_hashkey,
    {{ hash_column(acnt_contract_business_key_cols, source_name) }} AS acnt_contract_hashkey,
    to_date('{{ var("target_date") }}', 'yyyyMMdd') AS source_event_date,
    CONCAT(CAST('{{ source_name }}' AS string), '__', '{{ source_table }}') AS record_source,
    CAST(current_timestamp AS timestamp) AS load_timestamp
FROM {{ ref('v_stg_way4_balance') }} b
