...
*/
{{ config(
    alias = 'link_billing_acnt_contract',
    materialized = 'incremental_checkpoint',
    incremental_strategy = 'merge',
    unique_key = ['link_billing_acnt_contract_hashkey'],
...
{% set source_model = 'v_stg_way4_billing' %}
{% set source_name = 'way4' %}
{% set source_table = 'billing' %}
{% set unique_key = 'link_billing_acnt_contract_hashkey' %}
{% set source_business_key_cols = ['b.id', 'b.acnt_contract_id'] %}
{% set billing_business_key_cols = ['b.id'] %}
{% set acnt_contract_business_key_cols = ['b.acnt_contract_id'] %}
...
SELECT
    {{ hash_column(source_business_key_cols, source_name) }} AS link_billing_acnt_contract_hashkey,
    {{ hash_column(billing_business_key_cols, source_name) }} AS billing_hashkey,
    {{ hash_column(acnt_contract_business_key_cols, source_name) }} AS acnt_contract_hashkey,
    to_date('{{ var("target_date") }}', 'yyyyMMdd') AS source_event_date,
    CONCAT(CAST('{{ source_name }}' AS string), '__', '{{ source_table }}') AS record_source,
    CAST(current_timestamp AS timestamp) AS load_timestamp
FROM {{ ref('v_stg_way4_billing') }} b
JOIN {{ ref('v_stg_way4_acnt_contract') }} ac
    ON ac.id = b.acnt_contract_id
    AND ac.amnd_state = 'A'
    WHERE b.source_event_date = to_date('{{ var("target_date") }}', 'yyyyMMdd')
