...
*/
{{ config(
    alias = 'link_line_movement_toanhang_categ_entry',
    materialized = 'incremental_checkpoint',
    incremental_strategy = 'merge',
    unique_key = ['link_line_movement_toanhang_categ_entry_hashkey'],
...
{% set unique_key = 'link_line_movement_toanhang_categ_entry_hashkey' %}
...
{%- set raw_sql -%}
SELECT DISTINCT
    {{ hash_column(['t_line_id', 't_stt', 't_y_key_det'], source_name) }} AS link_line_movement_toanhang_categ_entry_hashkey,
    {{ hash_column(['t_line_id', 't_stt'], source_name) }} AS line_movement_toanhang_hashkey,
    {{ hash_column(['t_y_key_det'], source_name) }} AS categ_entry_hashkey,
    source_event_date,
    CONCAT(CAST('{{ source_name }}' AS string), '__', '{{ source_table }}') AS record_source,
    CAST(current_timestamp AS timestamp) AS load_timestamp
FROM {{ ref(source_model) }}
WHERE source_event_date = to_date('{{ var("target_date") }}', 'yyyyMMdd')
  AND t_xref_nxt_version = 'CATEG.ENTRY'
