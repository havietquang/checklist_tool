...
*/
{{ config(
    alias = 'link_crb_loans_payment_due',
    materialized = 'incremental_checkpoint',
    incremental_strategy = 'merge',
    unique_key = ['link_crb_loans_payment_due_hashkey'],
...
{% set crb_business_key_cols = ['a.tieukhoan', 'a.gl'] %}
{% set loans_payment_due_business_key_cols = ['b.id'] %}

{%- set raw_sql -%}
SELECT DISTINCT
    {{ hash_column(source_business_key_cols,source_name) }} AS link_crb_loans_payment_due_hashkey,
...
JOIN {{ ref('v_stg_t24_t24_payment_due') }} b
    ON a.tieukhoan = b.id
