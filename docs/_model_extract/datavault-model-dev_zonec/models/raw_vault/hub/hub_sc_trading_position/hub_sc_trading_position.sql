...
*/
{{ config(
    alias = 'hub_sc_trading_position',
    materialized = 'incremental_checkpoint',
    incremental_strategy = 'merge',
    unique_key = ['sc_trading_position_hashkey'],
...
WITH unioned_source AS (
    SELECT
        hashkey AS sc_trading_position_hashkey,
        id AS business_key,
        source_event_date,
        CONCAT('{{ source_name }}', '__', 't24_sc_trading_position') AS record_source,
        load_timestamp,
        1 AS source_priority
    FROM {{ ref('v_stg_t24_t24_sc_trading_position') }}
    ...
    SELECT
        ...
        id AS business_key,
        source_event_date,
        CONCAT('{{ source_name }}', '__', 't24_sc_trans_pos_history') AS record_source,
        load_timestamp,
        2 AS source_priority
    FROM {{ ref('v_stg_t24_t24_sc_trans_pos_history') }}
    WHERE source_event_date = to_date('{{ var("target_date") }}', 'yyyyMMdd')
      AND id IS NOT NULL
),
deduped AS (
    SELECT
        sc_trading_position_hashkey,
        business_key,
        source_event_date,
        record_source,
        load_timestamp,
        row_number() OVER (
            PARTITION BY sc_trading_position_hashkey
