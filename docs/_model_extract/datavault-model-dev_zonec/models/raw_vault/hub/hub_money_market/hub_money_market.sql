...
*/
{{ config(
    alias = 'hub_money_market',
    materialized = 'incremental_checkpoint',
    incremental_strategy = 'merge',
    unique_key = ['money_market_hashkey'],
...
WITH unioned_source AS (
    SELECT
        hashkey AS money_market_hashkey,
        id AS business_key,
        source_event_date,
        CONCAT('{{ source_name }}', '__', 't24_money_market') AS record_source,
        load_timestamp,
        1 AS source_priority
    FROM {{ ref('v_stg_t24_t24_money_market') }}
...
deduped AS (
    SELECT
        money_market_hashkey,
        business_key,
        source_event_date,
        record_source,
        load_timestamp,
        row_number() OVER (
            PARTITION BY money_market_hashkey
