...
{{ config(
    ...
    unique_key = ['user_hashkey'],
    skip_matched_step = true,
    tags = ['t24', 'entity', 'phase2', 'phase1', 'all']
) }}

{% set source_name = 't24' %}
{% set unique_key = 'user_hashkey' %}

{% set raw_sql %}
WITH unioned_source AS (
    SELECT
        hashkey AS user_hashkey,
        id AS business_key,
        source_event_date,
        CONCAT('{{ source_name }}', '__', 't24_user') AS record_source,
        load_timestamp,
        1 AS source_priority
    FROM {{ ref('v_stg_t24_t24_user') }}
    ...
    SELECT
        ...
        id AS business_key,
        source_event_date,
        CONCAT('{{ source_name }}', '__', 't24_user_level_auth') AS record_source,
        load_timestamp,
        2 AS source_priority
    FROM {{ ref('v_stg_t24_t24_user_level_auth') }}
    WHERE source_event_date = to_date('{{ var("target_date") }}', 'yyyyMMdd')
      AND id IS NOT NULL
    ...
    SELECT
        ...
        id AS business_key,
        source_event_date,
        CONCAT('{{ source_name }}', '__', 't24_vmbh_teller_limit') AS record_source,
        load_timestamp,
        2 AS source_priority
    FROM {{ ref('v_stg_t24_t24_vmbh_teller_limit') }}
    WHERE source_event_date = to_date('{{ var("target_date") }}', 'yyyyMMdd')
      AND id IS NOT NULL
),
deduped AS (
