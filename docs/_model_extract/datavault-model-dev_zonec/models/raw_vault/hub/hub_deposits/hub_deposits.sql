...
WITH unioned_source AS (
    SELECT
        hashkey AS deposit_hashkey,
        id AS business_key,
        source_event_date,
        CONCAT('{{ source_name }}', '__', 't24_az_account') AS record_source,
        load_timestamp,
        1 AS source_priority
    FROM {{ ref('v_stg_t24_t24_az_account') }}
    ...
    SELECT
        {{ hash_column(['az_number'], source_name) }} AS deposit_hashkey,
        trim(CAST(az_number AS string)) AS business_key,
        source_event_date,
        CONCAT('{{ source_name }}', '__', 't24_transfer_deposit') AS record_source,
        load_timestamp,
        2 AS source_priority
    FROM {{ ref('v_stg_t24_t24_transfer_deposit') }}
    WHERE source_event_date = to_date('{{ var("target_date") }}', 'yyyyMMdd')
      AND az_number IS NOT NULL
),
deduped AS (
    SELECT
        deposit_hashkey,
