-- depends_on: {{ ref('v_stg_t24_t24_forex') }}
-- depends_on: {{ ref('hub_forex') }}

{{ config(
    alias = 'sts_hub_forex',
    materialized = 'incremental_checkpoint',
    incremental_strategy = 'merge',
    unique_key = ['forex_hashkey', 'source_event_date', 'cdc_status'],
...
{% set unique_key = 'forex_hashkey' %}
{% set source_business_key_cols = ['id'] %}
{% set hub_model = 'hub_forex' %}
