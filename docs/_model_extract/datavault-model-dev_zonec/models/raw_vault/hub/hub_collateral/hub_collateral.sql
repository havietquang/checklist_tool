...
*/
{{ config(
    alias = 'hub_collateral',
    materialized = 'incremental_checkpoint',
    incremental_strategy = 'merge',
    unique_key = ['collateral_hashkey'],
    skip_matched_step = true,
    tags = ['t24', 'collateral', 'phase1', 'all']
) }}

{% set source_name = 't24' %}
{% set unique_key = 'collateral_hashkey' %}

{% set raw_sql %}
WITH unioned_source AS (
    SELECT
        hashkey AS collateral_hashkey,
        id AS business_key,
        source_event_date,
        CONCAT('{{ source_name }}', '__', 't24_collateral') AS record_source,
        load_timestamp,
    ...
    SELECT
        ...
        id AS business_key,
        source_event_date,
        CONCAT('{{ source_name }}', '__', 't24_ocbh_coll_basel') AS record_source,
        load_timestamp,
        2 AS source_priority
    FROM {{ ref('v_stg_t24_t24_ocbh_coll_basel') }}
    WHERE source_event_date = to_date('{{ var("target_date") }}', 'yyyyMMdd')
      AND id IS NOT NULL
),
deduped AS (
    SELECT
        ...
        load_timestamp,
        row_number() OVER (
            PARTITION BY collateral_hashkey
