...
{{ config(
    alias = 'sat_acc_templ_information',
...
{% set hashdiff_col = 'hashdiff_acc_templ_information' %}
{% set hub_hashkey = 'acc_templ_hashkey' %}
{% set source_model = 'v_stg_way4_acc_templ' %}
{% set list_cols = ['acc_scheme__oid', 'from_acc_scheme', 'account_type__id', 'f_i', 'pcat', 'acat', 'reference_only', 'code', 'curr', 'account_name', 'fx_type', 'due_type', 'due_period', 'grace_period', 'due_to_work_day', 'repayment_percent', 'min_repayment', 'min_rq_repayment', 'ageing_tariff', 'is_am_available', 'balance_type', 'group_code', 'charge_for_open'] %}
{% set raw_sql %}
select
    hashkey as acc_templ_hashkey,
    hashdiff_acc_templ_information as hashdiff,
