{% macro bridge_append(bridge_cfg) %}
...
{% if step_date_filter not in ['=', '<='] %}
    {{ exceptions.raise_compiler_error("Invalid date_filter '" ~ step_date_filter ~ "' in bridge_append step '" ~ step_name ~ "'. Allowed values: '=', '<='.") }}
{% endif %}
{% if step_hub_date_filter not in ['=', '<='] %}
    {{ exceptions.raise_compiler_error("Invalid hub_date_filter '" ~ step_hub_date_filter ~ "' in bridge_append step '" ~ step_name ~ "'. Allowed values: '=', '<='.") }}
{% endif %}
 
{% if not link_table %}
    {{ exceptions.raise_compiler_error("Missing 'link_table' in bridge_append step '" ~ step_name ~ "'.") }}
{% endif %}
{% if not link_fk %}
    {{ exceptions.raise_compiler_error("Missing 'link_fk' in bridge_append step '" ~ step_name ~ "'.") }}
{% endif %}
{% if not step.get('link_join_from') %}
    {{ exceptions.raise_compiler_error("Missing 'link_join_from' in bridge_append step '" ~ step_name ~ "'.") }}
{% endif %}
{% if link_as in ns.link_aliases %}
    {{ exceptions.raise_compiler_error("Duplicate link alias '" ~ link_as ~ "' in bridge_append step '" ~ step_name ~ "'.") }}
{% endif %}
...
{% if not effsat_hashkey %}
    {{ exceptions.raise_compiler_error("Missing 'effsat_hashkey' in bridge_append step '" ~ step_name ~ "' because 'effsat_table' is provided.") }}
{% endif %}
...
{% if not hub_pk %}
    {{ exceptions.raise_compiler_error("Missing 'hub_pk' in bridge_append step '" ~ step_name ~ "' because 'hub_table' is provided.") }}
{% endif %}
{% if hub_as in ns.hub_aliases %}
    {{ exceptions.raise_compiler_error("Duplicate hub alias '" ~ hub_as ~ "' in bridge_append step '" ~ step_name ~ "'.") }}
{% endif %}
...
{% if join_type not in ['left', 'inner'] %}
    {{ exceptions.raise_compiler_error("Invalid join_type '" ~ join_type_raw ~ "' in bridge_append step '" ~ step.get('name', link_as) ~ "'. Allowed values: left, inner, left join, inner join.") }}
{% endif %}
...
{% if where_clause or ns.effsat_filters|length > 0 %}
where 1=1
{% if where_clause %}
  and {{ where_clause }}
{% endif %}
...
select * from bridge_result
where snapshot_date = to_date('{{ var("target_date") }}', 'yyyyMMdd')
