-- ============================================================
-- Source table  : .crm.crm_contact_status
-- Target tables : ref_crm_contact_status
-- Date range    : fullload {{start_date}}=20250101
-- ============================================================

DROP TEMPORARY TABLE IF EXISTS tmp_crm_contact_status; CREATE TEMPORARY TABLE tmp_crm_contact_status AS
SELECT
    contact_status_id, contact_status_name, status, position, kh_denhan_bh,
    kh_cosp_dexuat, kh_roibo, hd_tiengui_denhan, sn_khachhang, kh_hienhuu,
    taikichhoat, kh_expiredcard, chbh_ganden, cust_group
FROM IDENTIFIER({{catalog_sourcing}} || '.crm.crm_contact_status')
WHERE contact_status_id IS NOT NULL;

INSERT INTO IDENTIFIER({{catalog_cleaned}} || '.raw_vault.ref_crm_contact_status')
(Ref_hashkey, Ref_type, Ref_code, Ref_description, status, position, kh_denhan_bh, kh_cosp_dexuat,
 ...
 chbh_ganden, cust_group, hashdiff_full, source_event_date, Record_source, load_timestamp)
WITH src AS (
    SELECT
        ...
        chbh_ganden,
        cust_group,
        sha2(COALESCE(UPPER(TRIM(CAST(contact_status_id   AS string))), '') || '$' ||
    ...
    FROM tmp_crm_contact_status
    WHERE contact_status_id IS NOT NULL
    QUALIFY ROW_NUMBER() OVER (PARTITION BY contact_status_id ORDER BY 1) = 1
)
SELECT
    ...
    'crm__crm_contact_status' AS Record_source,
    current_timestamp() AS load_timestamp
FROM src d LEFT ANTI JOIN IDENTIFIER({{catalog_cleaned}} || '.raw_vault.ref_crm_contact_status') t
    ON t.Ref_hashkey = d.Ref_hashkey AND t.hashdiff_full = d.hashdiff_full;

DROP TEMPORARY TABLE IF EXISTS tmp_crm_contact_status;
