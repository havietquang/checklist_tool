-- ============================================================
-- Source  : .appsflyer.in_app_events_report
-- Targets : hub_appsflyer
--           non_his_sat_in_app_events_event
--           non_his_sat_in_app_events_device
...
-- ============================================================
DROP TEMPORARY TABLE IF EXISTS tmp_in_app_events_report; CREATE TEMPORARY TABLE tmp_in_app_events_report AS
SELECT
    ...
    city, wifi, operator, language, advertising_id, android_id, customer_user_id, imei, idfv,
...
FROM IDENTIFIER({{catalog_sourcing}} || '.appsflyer.in_app_events_report')
...
-- Satellite: non_his_in_app_events_event (append toàn bộ dòng, khớp model)
INSERT INTO IDENTIFIER({{catalog_cleaned}} || '.raw_vault.non_his_sat_in_app_events_event')
...
delta AS (
    ...
    SELECT
        ...
        reengagement_window,
        is_primary_attribution
    FROM IDENTIFIER({{catalog_cleaned}} || '.raw_vault.non_his_sat_in_app_events_event')
...
SELECT
    ...
    'appsflyer__in_app_events_report' AS record_source,
...
-- Satellite: non_his_in_app_events_device (append toàn bộ dòng, khớp model)
INSERT INTO IDENTIFIER({{catalog_cleaned}} || '.raw_vault.non_his_sat_in_app_events_device')
(appsflyer_hashkey, source_event_date, load_timestamp, record_source,
 city, wifi, operator, language, advertising_id, android_id, customer_user_id, imei, idfv,
 platform, device_type, os_version, app_version, sdk_version, app_id, app_name, bundle_id,
 country_code, dma, ip, postal_code, region, state, user_agent, http_referrer, original_url)
...
delta AS (
    ...
    SELECT
        ...
        http_referrer,
        original_url
    FROM IDENTIFIER({{catalog_cleaned}} || '.raw_vault.non_his_sat_in_app_events_device')
    WHERE source_event_date BETWEEN to_date({{start_date}}, 'yyyyMMdd') AND to_date({{end_date}}, 'yyyyMMdd')
)
SELECT
    d.appsflyer_hashkey AS appsflyer_hashkey,
...
DROP TEMPORARY TABLE IF EXISTS tmp_in_app_events_report;
