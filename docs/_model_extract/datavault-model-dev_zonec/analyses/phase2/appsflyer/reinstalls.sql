-- ============================================================
-- Source  : .appsflyer.reinstalls
-- Targets : hub_appsflyer
--           sat_reinstalls_event
--           sat_reinstalls_device
-- Range   : 20250101 -> 20250131
-- ============================================================
DROP TEMPORARY TABLE IF EXISTS tmp_reinstalls; CREATE TEMPORARY TABLE tmp_reinstalls AS
SELECT
    ...
    sha2(
        ...
        COALESCE(UPPER(TRIM(CAST(is_primary_attribution         AS string))), ''), 256) AS hd_reinstalls_event,
    sha2(
        ...
        COALESCE(UPPER(TRIM(CAST(original_url   AS string))), ''), 256) AS hd_reinstalls_device,
...
FROM IDENTIFIER({{catalog_sourcing}} || '.appsflyer.reinstalls')
...
-- Satellite: reinstalls_event
INSERT INTO IDENTIFIER({{catalog_cleaned}} || '.raw_vault.sat_reinstalls_event')
(appsflyer_hashkey, hashdiff, source_event_date, load_timestamp, record_source,
 ...
 is_retargeting, retargeting_conversion_type, attribution_lookback, reengagement_window, is_primary_attribution)
WITH deduped AS (
    SELECT * FROM tmp_reinstalls
    QUALIFY ROW_NUMBER() OVER (PARTITION BY appsflyer_hashkey, hd_reinstalls_event ORDER BY source_event_date) = 1
)
SELECT
    d.appsflyer_hashkey AS appsflyer_hashkey,
    d.hd_reinstalls_event AS hashdiff,
    d.source_event_date AS source_event_date,
    current_timestamp() AS load_timestamp,
    'appsflyer__reinstalls' AS record_source,
...
FROM deduped d
LEFT ANTI JOIN IDENTIFIER({{catalog_cleaned}} || '.raw_vault.sat_reinstalls_event') t
    ON t.appsflyer_hashkey = d.appsflyer_hashkey AND t.hashdiff = d.hd_reinstalls_event;

-- Satellite: reinstalls_device
INSERT INTO IDENTIFIER({{catalog_cleaned}} || '.raw_vault.sat_reinstalls_device')
(appsflyer_hashkey, hashdiff, source_event_date, load_timestamp, record_source,
 city, wifi, operator, language, advertising_id, idfa, android_id, customer_user_id, imei, idfv,
 platform, device_type, os_version, app_version, sdk_version, app_id, app_name, bundle_id,
 country_code, dma, ip, postal_code, region, state, user_agent, http_referrer, original_url)
WITH deduped AS (
    SELECT * FROM tmp_reinstalls
    QUALIFY ROW_NUMBER() OVER (PARTITION BY appsflyer_hashkey, hd_reinstalls_device ORDER BY source_event_date) = 1
)
SELECT
    d.appsflyer_hashkey AS appsflyer_hashkey,
    d.hd_reinstalls_device AS hashdiff,
    d.source_event_date AS source_event_date,
    current_timestamp() AS load_timestamp,
    'appsflyer__reinstalls' AS record_source,
    d.city AS city,
...
FROM deduped d
LEFT ANTI JOIN IDENTIFIER({{catalog_cleaned}} || '.raw_vault.sat_reinstalls_device') t
    ON t.appsflyer_hashkey = d.appsflyer_hashkey AND t.hashdiff = d.hd_reinstalls_device;

DROP TEMPORARY TABLE IF EXISTS tmp_reinstalls;
