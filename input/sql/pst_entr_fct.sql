...
-- ── satellite snapshot: lấy bản ghi mới nhất tính đến ngày chạy ──────────────
sat_usr_info AS (
    SELECT user_hashkey,
...
-- sat_funds_transfer_system thuộc 49 bảng transaction -> lọc = :DATADT
sat_ft_sys AS (
...
-- sat_teller_other thuộc nhóm bảng transaction -> lọc = :DATADT
sat_tlr_other AS (
...
-- ── link snapshot: rút về current (1 dòng / driving key) chống fan-out ───────
lnk_ft_branch AS (
    SELECT funds_transfer_hashkey,
...
-- hub_line_movement_toanhang: snapshot 1 lần, dùng chung cho cả 3 loại bút toán
line_mv_hub AS (
...
-- link của bút toán: bảng transaction -> = :DATADT,
-- cộng nhánh toa nhang (bút toán ghi nhận GL đúng ngày chạy nhưng phát sinh trước đó)
lnk_stmt_branch AS (
...
-- satellite bút toán: snapshot 1 lần, giữ đủ dòng mà logic cũ cần
-- (dòng của ngày chạy + mọi dòng lịch sử của bút toán có ghi nhận GL đúng ngày chạy)
stmt_info_raw AS (
